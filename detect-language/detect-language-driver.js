...
class DetectLanguageDriver extends TextAnalyticsBase {

    constructor(node, key, region) {
        super(node, key, region);
        this.baseUrl = `https://${this.getRegion()}.api.cognitive.microsoft.com/text/analytics/v3.2-preview.2/languages`;
    }
    ...
    preProcess(options) {
        ...
        const availableSet = new Set(['id', 'text', 'countryHint']);
        for (let obj of options.requestArray) {
            ...
            for (let p of ownProperties) {
                ...
                throw new Error(`Property '${p}' is not allowed. Only 'id', 'text' and 'countryHint' are allowed.`);
            }
        }
...
module.exports = DetectLanguageDriver;
