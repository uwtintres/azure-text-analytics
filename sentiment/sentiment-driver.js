...
class SentimentDriver extends TextAnalyticsBase {

    constructor(node, key, region) {
        super(node, key, region);
        this.baseUrl = `https://${this.getRegion()}.api.cognitive.microsoft.com/text/analytics/v3.2-preview.2/sentiment`;
    }
    ...
    async analyzeInternal({ showStats, loggingOptOut, opinionMining, stringIndexType, requestBody, modelVersion, config }) {
        this.setStatus({ fill: 'green', shape: 'dot', text: 'Detecting' });
        const res = await axios.post(`${this.baseUrl}?showStats=${showStats}&loggingOptOut=${loggingOptOut}&opinionMining=${opinionMining}&stringIndexType=${stringIndexType}&modelVersion=${modelVersion}`, requestBody, config);
        return res.data;
    }
}

module.exports = SentimentDriver;
